...
public class Model {
    ...
    public int minBankCashAssets() {
        return accounts.stream().mapToInt(Account::getBalance).min().orElse(0);
    }

    public int maxBankCashAssets() {
        return accounts.stream().mapToInt(Account::getBalance).max().orElse(0);
    }

    public float avgBankCashAssets() {
        return (float) accounts.stream().mapToInt(Account::getBalance).sum() / accounts.size();
    }
